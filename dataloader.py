...
class HarborfrontClassificationDataset():
    ...
    def __init__(self, data_split, root, classes=CLASS_LIST.values(), binary_cls=True, verbose=False) -> None:
        ...
        if binary_cls:
            self.dataset["target"] = self.dataset.apply(lambda x: np.asarray([1 if int(x[g]) > 0 else 0 for g in self.classes]), axis=1)
        else:
            self.dataset["target"] = self.dataset.apply(lambda x: np.asarray([int(x[g]) for g in self.classes]), axis=1)

        if verbose:
            print(f'Successfully loaded "{data_split}" as {self.__repr__()}')

    def get_tf_dataloader(self, batch_size=8):
        images,labels  = self.dataset.apply(lambda x: tf.io.read_file(x["file_name"]).values(), axis=1), self.dataset["target"].values()
        dataset = tf.data.Dataset.from_tensor_slices(zip(images, labels), batch_size=batch_size)
    ...
    def get_data_generator(self, batchsize=8):
        ...
        #img_preprocessing.fit(np.random.rand(1,288,384))
