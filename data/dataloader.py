...
class HarborfrontClassificationDataset():
    ...
    def __init__(self, data_split, root, classes=CLASS_LIST.values(), resize=(512,512), binary_cls=True, verbose=False) -> None:
        if verbose:
        ...
        self.preprocesses =[
            ...
            tf.keras.layers.Resizing(resize[0],resize[1])
        ]
        self.augmentations=[
        ...
        #Remove NA
        self.dataset.fillna(value=0, inplace=True)
    ...
    def load_and_preprocess_from_path_label(self, path, label):
        return self.load_and_preprocess_image(path), tf.constant(np.asanyarray(label))

    def get_dataloader(self, batchsize=8, shuffle_data=True):
        #Load from dataset
        ds = tf.data.Dataset.from_tensor_slices((self.dataset["file_name"].to_numpy(), self.dataset["target"].to_numpy()))
...
if __name__ == "__main__":
    import numpy as np
    dataset = HarborfrontClassificationDataset("data/Test_data.csv", "/Data/Harborfront_raw/")
    print(dataset)
